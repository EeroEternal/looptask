  ...
  return (
    <div className="workspace-shell">
      ...
      <main className="workspace-main">
        <div className="intro-row"><div><p className="overline">工作区 / 新任务</p><h1>让一个任务跑起来。</h1><p className="intro-copy">连接仓库，描述目标，写下完成的证据，然后发送到 Agent cell。</p></div><div className="github-note"><span className="github-symbol">GH</span><div><strong>GitHub App 尚未授权</strong><small>当前仅能保存并派发任务；授权后才能自动创建 PR 和发送确认邮件。</small></div><button onClick={() => setGithubOpen(true)} type="button">了解</button></div></div>
        ...
        <section className="task-layout">
          <div className="task-card">
            ...
            <div className="form-section"><p className="section-number">01 <span>项目</span></p><div className="field-grid"><Field label="项目名称"><input value={project} onChange={(e) => setProject(e.target.value)} placeholder="例如 looptask" /></Field><Field label="默认分支"><input className="mono" value={branch} onChange={(e) => setBranch(e.target.value)} placeholder="main" /></Field></div><Field label="GitHub 仓库地址"><input type="url" value={repository} onChange={(e) => setRepository(e.target.value)} placeholder="https://github.com/owner/repository" /></Field></div>
          ...
          <aside className="task-aside"><div className="aside-card github-card"><p className="overline">GitHub connection</p><h3>自动化还差两步</h3><p>当前任务只会进入 Agent cell。连接执行器和 GitHub App 后，looptask 才能修改仓库、运行验收、创建 pull request，并发送确认邮件。</p><button onClick={() => setGithubOpen(true)} type="button">查看当前能力 <span>→</span></button></div><div className="aside-card"><p className="overline">当前配置</p><div className="config-line"><span>仓库</span><strong>{repository || "尚未填写"}</strong></div><div className="config-line"><span>分支</span><strong>{branch || "main"}</strong></div><div className="config-line"><span>运行方式</span><strong>{schedule === "manual" ? "手动一次" : `常驻 · ${formatInterval(residentInterval)}`}</strong></div><div className="config-line"><span>验收命令</span><strong>{verifiers.filter((v) => v.command.trim()).length} 条</strong></div></div></aside>
        </section>

        <section className="runs-card"><div className="runs-heading"><div><p className="overline">Recent runs</p><h2>最近派发</h2></div>{currentRun && <span className="latest-label">最新 · {statusLabel(currentRun.status)}</span>}</div>{runs.length ? <div className="run-list">{runs.slice(0, 6).map((run) => <button className="run-item" key={run.id} onClick={() => void loadEvents(run.id)} type="button"><span className={`run-dot ${run.status}`} /><span className="run-copy"><strong>{run.projectName || run.project || project} <em>/</em> {run.loopName || loopName}</strong><small>{run.startedAt ? formatDate(run.startedAt) : `运行 ${run.id}`}</small></span><b>{statusLabel(run.status)}</b><span className="run-arrow">→</span></button>)}</div> : <div className="runs-empty"><span className="empty-line" /><strong>还没有派发记录</strong><p>配置好第一个任务后，它会出现在这里。</p></div>}{events.length > 0 && <div className="events-preview"><strong>派发事件</strong>{events.slice(-3).map((event, index) => <span key={event.id || index}>{event.eventType || "event"}</span>)}</div>}</section>
        {schedule === "resident" && <button className="stop-resident" disabled={!!busy} onClick={() => void stopResident()} type="button">停止当前常驻任务</button>}
      </main>
      {githubOpen && <div className="simple-modal-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) setGithubOpen(false); }}><div className="simple-modal" role="dialog" aria-modal="true" aria-labelledby="github-title"><button className="modal-close" onClick={() => setGithubOpen(false)} type="button">×</button><span className="github-symbol large">GH</span><h2 id="github-title">当前只完成任务派发</h2><p>任务可以保存、校验并发送到 Agent cell，但真正修改仓库、执行验收、创建 PR 和发送邮件，还需要接入执行器与 GitHub App。</p><div className="modal-note">连接入口将在 GitHub App 配置完成后开放。looptask 不会把“已入队”伪装成“已完成”。</div><button className="run-button" onClick={() => setGithubOpen(false)} type="button">知道了</button></div></div>}
